...
async function findAvailablePort(startPort, endPort = startPort + 1000) {
...
async function findReferencesWithTypeScript(options) {
    const references = [];
    const processedFiles = new Set();
    const configPath = ts.findConfigFile(options.directory, ts.sys.fileExists, "tsconfig.json");
    if (!configPath) {
        console.log("No tsconfig.json found, using default settings");
        return searchWithoutConfig(options);
    }
    const { config } = ts.readConfigFile(configPath, ts.sys.readFile);
    const { options: compilerOptions } = ts.parseJsonConfigFileContent(config, ts.sys, path.dirname(configPath));
            ...
            if (ts.isIdentifier(node) && node.text === options.word) {
                ...
                if (symbol) {
                    ...
                    if (isDefinition && options.isSymbolSearch) {
                        const refs = services.findReferences(sourceFile.fileName, node.getStart());
                        if (refs) {
                            refs.forEach((referenceEntry) => {
                                referenceEntry.references.forEach((ref) => {
...
async function searchWithoutConfig(options) {
    const references = [];
...
async function findReferencesWithGrep(word, directory) {
    try {
        ...
        if (tools.ripgrep) {
            ...
            const exactMatchSet = new Set(exactMatches.map(match => `${match.file}:${match.line}:${match.text}`));
...
async function searchWithRipgrep(word, directory, exactMatch) {
    return new Promise((resolve, reject) => {
        ...
        const rgProcess = (0, child_process_1.spawn)(rgPath, rgArgs);
        let output = '';
        let errorOutput = '';
...
async function fallbackToGrep(word, directory) {
    try {
        ...
        const exactMatchSet = new Set(exactMatches.map(match => `${match.file}:${match.line}:${match.text}`));
        partialMatches = partialMatches.filter(match => !exactMatchSet.has(`${match.file}:${match.line}:${match.text}`));
        return [...exactMatches, ...partialMatches];
    }
    catch (error) {
...
function parseGrepOutput(output, isExactMatch) {
    const locations = [];
...
function combineSearchResults(exactMatches, partialMatches) {
    // Create a Set of unique identifiers for exact matches
    const exactMatchSet = new Set(exactMatches.map(match => `${match.file}:${match.line}:${match.column}`));
    // Filter out partial matches that are already in exact matches
    const uniquePartialMatches = partialMatches.filter(match => !exactMatchSet.has(`${match.file}:${match.line}:${match.column}`));
    // Combine exact matches first, then partial matches
    return [...exactMatches, ...uniquePartialMatches];
}
async function findReferencesWithNode(word, baseDir) {
    const locations = [];
    const processedFiles = new Set();
...
function enhanceLocation(location, baseDir) {
...
function groupReferencesByFileType(references) {
    ...
    if (req.method === 'POST' && req.url === '/find-references') {
        ...
        req.on('end', async () => {
            try {
                ...
                const enhancedReferences = references.map(ref => enhanceLocation(ref, request.directory));
                const uniqueRefs = Array.from(new Map(enhancedReferences.map(ref => [`${ref.relativePath}:${ref.line}:${ref.column}`, ref])).values());
